...
class ConfigFileWriter:
                ...
                if key_name in new_values:
                    ...
                    if not isinstance(new_values[key_name], dict):
                        ...
                        j = self._update_subattributes(
                            j, contents, new_values[key_name], len(match.group(1)) - len(match.group(1).lstrip())
                        )
                        return
            j += 1
        ...
        for i in range(index, len(contents)):
            ...
            if match is not None:
                ...
                if key_name in values:
                    ...
                    new_line = "%s%s = %s\n" % (" " * current_indent, key_name, option_value)
                    contents[i] = new_line
                    del values[key_name]
    ...
    def _matches_section(self, match, section_name):
        ...
        if len(parts) > 1:
            quoted_match = match.group(0) == '[%s "%s"]' % (parts[0], " ".join(parts[1:]))
