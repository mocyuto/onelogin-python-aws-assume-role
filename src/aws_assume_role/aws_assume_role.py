...
def get_options():
    ...
    if not options.saml_api_version or options.saml_api_version < 1:
...
def get_saml_response(
    ...
    while saml_endpoint_response is None or saml_endpoint_response.type == "pending":
        if saml_endpoint_response is None:
            if client.error in ["400", "401"]:
                ...
                elif client.error_description in [
        ...
        if try_get_saml_response == MAX_ITER_GET_SAML_RESPONSE:
            print(
                "Not able to get a SAMLResponse with success status after %s iteration(s)." % MAX_ITER_GET_SAML_RESPONSE
            )
            sys.exit()
    ...
    if saml_endpoint_response and saml_endpoint_response.type == "success":
        if saml_endpoint_response.mfa is not None:
            ...
            if mfa_verify_info is None or device_type in ["OneLogin SMS", "OneLogin Protect"]:
                ...
                if device_type == "OneLogin SMS":
                    ...
                    saml_endpoint_response = client.get_saml_assertion_verifying(
                        app_id, device_id, state_token, None, do_not_notify=True
                    )
                    ...
                    if "otp_token" not in mfa_verify_info:
                        saml_endpoint_response = client.get_saml_assertion_verifying(
                            app_id, device_id, state_token, None, do_not_notify=False
                        )
                        print("PUSH with OTP token sent to device %s" % device_id)
                    while (
                        saml_endpoint_response
                        and saml_endpoint_response.type == "pending"
                        and try_get_saml_response_verified < MAX_ITER_GET_SAML_RESPONSE
                    ):
                        time.sleep(TIME_SLEEP_ON_RESPONSE_PENDING)
                        saml_endpoint_response = client.get_saml_assertion_verifying(
                            app_id, device_id, state_token, None, do_not_notify=True
            ...
            if not verified_with_push:
                saml_endpoint_response = client.get_saml_assertion_verifying(
                    app_id, device_id, state_token, otp_token, do_not_notify=True
                )
                ...
                while client.error or saml_endpoint_response is None:
                    if client.error_description == "State token is invalid or expired":
                        ...
                        if mfa_error > MFA_ATTEMPTS_FOR_WARNING and len(devices) > 1:
                            print(
                                "The OTP Token was not able to be processed after %s attempts, Do you want to select a new MFA method? (y/n)"
                                % MFA_ATTEMPTS_FOR_WARNING
                            )
...
def get_attributes(saml_response):
    ...
    NSMAP = {"samlp": "urn:oasis:names:tc:SAML:2.0:protocol", "saml": "urn:oasis:names:tc:SAML:2.0:assertion"}
    attributes = {}
...
def get_data_from_cache():
    ...
    with open(SAML_CACHE_PATH) as f:
...
def write_data_to_cache(contents):
    ...
    """
    print(f"Writing SAML cache to {SAML_CACHE_PATH}")
...
def main():
    ...
    for i in iterations:
        ...
        missing_onelogin_data = (
            username_or_email is None or password is None or app_id is None or onelogin_subdomain is None
        )
        ...
        if i == 0 or ask_for_role_again:
            ...
            if "https://aws.amazon.com/SAML/Attributes/Role" not in attributes.keys():
                ...
                if len(roles) > 1:
                    ...
                    selection_info, role_option = process_account_and_role_choices(
                        info_indexed_by_account, info_indexed_by_roles, options
                    )
                    ...
                    if role_option is None:
                        if options.aws_account_id and options.aws_role_name:
                            print(
                                "SAMLResponse from Identity Provider does not contain available AWS Role: %s for AWS Account: %s"
                                % (options.aws_role_name, options.aws_account_id)
                            )
                    ...
                    if data[0] == "Default" or not data[1]:
                        print(
                            "SAMLResponse from Identity Provider does not contain available AWS Account/Role for this user"
                        )
                        if ask_iteration_new_user():
                            ask_for_user_again = True
                            result = None
        ...
        try:
            ...
            aws_session_token = conn.assume_role_with_saml(
                RoleArn=role_arn, PrincipalArn=principal_arn, SAMLAssertion=saml_response, DurationSeconds=duration
            )
            ...
            if (
                "Token must be redeemed within 5 minutes of issuance" in error_msg
                or "An error occurred (ExpiredTokenException) when calling the AssumeRoleWithSAML operation"
                in error_msg
            ):
                print(error_msg)
                ...
                print(error_msg)
                print(
                    "Introduce a new value, to be used on this Role, for DurationSeconds between 900 and 43200. Previously was %s"
                    % duration
                )
        ...
        if options.interactive:
            print(
                "\n\nThe process regenerated credentials for user %s with AWS Role %s "
                % (username_or_email, selected_role)
            )
